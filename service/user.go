...
// checkUser: whether user is valid
func checkUser(username, password string) bool {
	es_client, err := elastic.NewClient(elastic.SetURL(ES_URL), elastic.SetSniff(false))
	if err != nil {
		fmt.Print("ES is not setup %v\b", err)
		return false
	}
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if u.Username != "" && u.Password != "" && usernamePattern(u.Username) {
		...
		//w.Write([]byte("Empty password or username or invalid username."), http.StatusInternalServerError)
	}
